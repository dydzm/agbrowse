...
| Adaptive URL fetch (`agbrowse fetch <url>`) | experimental | `skills/browser/adaptive-fetch/*.mjs` (index, safety, trace, fetcher, endpoint-resolvers, metadata, transforms, reader-adapters, content-scorer, third-party-readers, challenge-detector, waf-profiles, tls-fetch, browser-escalation, browser-runtime, browser-session, human-loop, output), `skills/browser/browser.mjs` | `test/unit/browser-adaptive-fetch-*.test.mjs`, `test/integration/browser-fetch-command.test.mjs` including large JSON stdout compaction | v2: 6-phase escalation ladder (public endpoints → browser-grade HTTP → readers → isolated browser → user session → human resolution). Session modes: none/isolated/existing/user/interactive. Identity modes: auto/minimal/chrome. WAF profile detection (Cloudflare managed+Turnstile, Akamai Bot Manager, AWS WAF, Imperva/Incapsula, DataDome, PerimeterX). TLS/JA3 impersonation fallback via curl-impersonate (203.1) on 403/429/challenge before browser escalation. DNS rebinding guard. Content scoring with multi-signal evidence. JSON output content is compacted with `contentBytes`/`contentLimitBytes`/`contentTruncated`. planned as `cli-jaw browser fetch <url>` after agbrowse v2 proves stable. |
...
| Schema-bound CLI extraction (`agbrowse extract`, envelope `agbrowse-extract-v1`) | experimental | `skills/browser/extract.mjs`, `skills/browser/browser.mjs` (`case 'extract'`) | `test/unit/browser-extract-mapping.test.mjs`, `test/integration/extract-cli.test.mjs` | Tier 1 LLM-free HTML-to-schema pipeline + Tier 2 `--escalate-web-ai` opt-in. Fail-closed: `no_mappable_structure` verdict on mapping failure, no silent partial data. JS-rendered pages are Tier 1 non-goal. cli-jaw does NOT mirror — extraction stays first-party in agbrowse. |
