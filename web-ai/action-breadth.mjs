// @ts-check
/**
 * G03 — action breadth catalog (pure).
 *
 * Single source of truth for the local-CDP browser primitive surface.
 * Used by `gate:browser-primitives-complete` to assert that the CLI
 * actually wires each primitive listed here, and by docs/tests for
 * parity tracking against external agent browsers (Vercel agent-browser,
 * Playwright MCP) — without crossing the frozen MCP scope.
 *
 * No hosted/cloud, no stealth, no CAPTCHA bypass, no external CDP.
 */

/**
 * @typedef {Object} BrowserPrimitive
 * @property {string} command           CLI subcommand name (matches `case 'X'` in skills/browser/browser.mjs)
 * @property {string} category          high-level grouping for docs
 * @property {string} description       what the primitive does
 * @property {string[]} args            ordered positional argument names
 * @property {string[]} [flags]         supported optional flags (e.g. --json)
 * @property {string} [requiresRef]     if the primitive operates on a ref
 */

/** @type {readonly BrowserPrimitive[]} */
export const BROWSER_PRIMITIVES = Object.freeze([
 ...
 * @returns {string[]}
 */
export function listPrimitiveCommands() {
    return BROWSER_PRIMITIVES.map(p => p.command);
}

/**
 * Group primitives by category.
 * @returns {Record<string, BrowserPrimitive[]>}
 */
export function primitivesByCategory() {
    /** @type {Record<string, BrowserPrimitive[]>} */
    const out = {};
...
/**
 * Audit a CLI source string and report which primitives are wired
 * (i.e. have a `case '<command>':` handler) and which are missing.
 * @param {string} source
 */
export function auditPrimitiveCoverage(source) {
    const found = [];
    const missing = [];
    for (const p of BROWSER_PRIMITIVES) {
...
export const BROWSER_PRIMITIVE_SCHEMA_VERSION = 'browser-primitives-v1';
