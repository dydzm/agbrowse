...
| 03 response capture | 🟡 OPEN | 이중 경로 race + 3차 복구 미구현 (`MutationObserver` 0건) |
| 04 session reattach | 🟡 PARTIAL | `sessions reattach`/`resume` 구현; sidebar·DR-reattach만 OPEN |
| 05 error taxonomy | 🟡 OPEN | 서브클래스 미구현(저긴급); errorCode 36개 |
| 06 dom diagnostics | 🟡 OPEN | 실패 시 자동 DOM/스크린샷 캡처 없음 |
...
**즉시 구현 착수 가능 backlog**: 31(P0 generic file lane) · 32.1(P0 DR capture) · 32.3(P1 session guard) · 32.2(P1 model picker, 감사 완료) · 03(P1 dual-path) · 06(P2 diagnostics).
...
5. 🟡 **에러 분류**: (OPEN, 저긴급) oracle 3-tier vs agbrowse 단일 `WebAiError`; errorCode 36개
